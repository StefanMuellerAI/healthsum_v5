...
from models import User, Report, ReportTemplate, TaskMonitor as NewTaskMonitor
import os
...
new_db_path = os.path.join('instance', 'health_records_new.db')
...
class OldHealthRecord(Base):
    ...
    filenames = Column(Text)  # In der alten DB nicht verschlüsselt
    ...
    try:
        with app.app_context():
            ...
            for old_record in old_session.query(OldHealthRecord).all():
                from models import HealthRecord  # Import hier um Circular Import zu vermeiden
                new_record = HealthRecord(
                    id=old_record.id,
                    timestamp=old_record.timestamp,
                    text=old_record.text,
                    filenames=old_record.filenames,  # Wird automatisch verschlüsselt
                    ...
                    custom_instructions=None  # Neue Spalte mit Standardwert
                )
